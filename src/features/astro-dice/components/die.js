...
export function dieMarkup(die) {
  return `<div class="die-position die-${die.id}"><div class="astro-die" data-die="${die.id}" role="img" aria-label="${die.label}骰，等待投掷"><img src="${sitePath(`assets/dice/${die.id}.svg`)}" width="160" height="172" alt=""><span class="die-symbol" aria-hidden="true">${die.faces[0].symbol}</span></div><span class="die-caption">${die.label}</span></div>`;
}
